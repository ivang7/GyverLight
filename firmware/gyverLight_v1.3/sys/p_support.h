    ...
    typedef std::function<void(String name, String value)> callBackParamChanged;
    callBackParamChanged callback;

    void html_pages(callBackParamChanged callbackFunction)
    {
    ...
    void page_post()
    {
      server.on("/post", HTTP_ANY, [](AsyncWebServerRequest *request) {
        ...
        for (int i = 0; i < params; i++)
        {
          ...
          if(callback != NULL) callback(p->name(), p->value());
